...
class MedianFilter2D(nn.Module):
    ...
    This module extends `torch.nn.Module` and can be integrated into neural networks
    to perform median filtering as part of the network's forward pass.

    Parameters
    ----------
    kernel_size : int
        Size of the neighborhood from which the median is computed. Must be an odd number.
    ...
    def forward(self, image):
        """
        Apply the median filter to an input image as part of the model's forward pass.

        Parameters
        ----------
        image : torch.Tensor
            Input 2D tensor representing the image. Should be of shape [height, width] or
            [batch_size, channels, height, width] for batches of images.

        Returns
        -------
        torch.Tensor
            Output 2D tensor after applying the median filter. Shape matches the input shape.
        """
        # Ensure image tensor is at least 4D (batch_size, channels, height, width)
        if image.dim() == 2:
        ...
        patches = padded_image.unfold(2, self.kernel_size, 1).unfold(3, self.kernel_size, 1)
        patches = patches.contiguous().view(*image.shape[:2], *patches.shape[-4:-2], -1)
...
class MedianFilter3D(MedianFilter2D):
    ...
    Inherits
    --------
    MedianFilter2D : An implementation of 2D median filtering as a PyTorch module.
    ...
    def forward(self, volume):
        """
        Apply the median filter to an input 3D volume as part of the model's forward pass.

        Parameters
        ----------
        volume : torch.Tensor
            Input 3D tensor representing the volume. Should be of shape [depth, height, width]
            or [batch_size, channels, depth, height, width] for batches of volumes.

        Returns
        -------
        torch.Tensor
            Output 3D tensor after applying the median filter. Shape matches the input shape.
        """
        # Handle different dimensions of input
        if volume.dim() == 3:
            volume = volume.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimension
        elif volume.dim() == 4:
    ...
    def median_filter(self, slice):
        """
        Helper function to apply 2D median filtering to each slice of the volume.
        ...
        """
        return super().forward(slice)  # Use the forward method from MedianFilter2D
...
class SobelFilter2D:
    ...
    def apply_filter(self, image):
        ...
        edge_x = F.conv2d(image, self.sobel_x.to(**backend), padding=self.kernel_size//2)
        edge_y = F.conv2d(image, self.sobel_y.to(**backend), padding=self.kernel_size//2)
...
class VarianceFilter2D(nn.Module):
    """
    A PyTorch module to apply a variance filter to 2D images using a specified window size.
    """
    ...
    def __init__(self, window_size):
        ...
        ----------
        window_size : int
            Size of the window to compute the variance. Must be an odd number to maintain symmetry.
        """
        super().__init__()
        if window_size % 2 == 0:
    ...
    def forward(self, image):
        ...
        """
        if image.dim() == 2:
            image = image.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions if not present
        elif image.dim() == 3:
            image = image.unsqueeze(1)  # Add channel dimension

        # Padding for same output size
        image = F.pad(image, (self.pad, self.pad, self.pad, self.pad), mode='reflect')
        # Calculate local mean and local mean square
        kernel = torch.ones((1, 1, self.window_size, self.window_size), device=image.device, dtype=torch.float32) / (self.window_size**2)
...
class VarianceFilter2D:
    """
    Class to apply a variance filter to a 2D image.
    """

    def __init__(self, window_size):
        """
        Initialize VarianceFilter2D with a specific window size.

        Parameters
        ----------
        window_size : int
            Size of the window to compute the variance.
        """
        self.window_size = window_size
    ...
    def apply_filter(self, image):
        ...
        -------
        torch.Tensor
            A 2D tensor of local variance values, maintaining same backend as input.
        """
        backend = dict(device=image.device, dtype=image.dtype)
        image = image.to(torch.float32).cpu().numpy()

        # Compute local mean and squares
        local_mean = scipy.ndimage.uniform_filter(image, size=self.window_size)
        local_mean_squares = scipy.ndimage.uniform_filter(image**2, size=self.window_size)

        # Calculate variance
        local_variance = local_mean_squares - local_mean**2
        return torch.from_numpy(local_variance).to(**backend)


class VarianceFilter3D(VarianceFilter2D):
    """
    Class to apply a variance filter to a 3D image by extending VarianceFilter2D.
    """
    ...
    def apply_filter(self, volume):
        ...
        -------
        torch.Tensor
            A 3D tensor of local variance values, with variance computed along the smallest dimension.
        """
        smallest_dim = volume.size().index(min(volume.size()))
...
class GaborFilter:
    def __init__(self, sizes=[31], sigmas=[4], thetas=[1], lambdas=[10], psi=[0], gammas=[0.5]):
        """
        Initialize the Gabor filter with specified parameters for multiple sizes and wavelengths.
        Generates multiple kernels based on combinations of sizes and wavelengths.
        ...
        for size, lambda_ in zip(sizes, lambdas):
            for sigma, theta, psi, gamma in zip(sigmas, thetas, psis, gammas):
                kernel = self.generate_gabor_kernel(size, sigma, theta, lambda_, psi, gamma)
    ...
    def generate_gabor_kernel(self, size, sigma, theta, lambda_, psi, gamma):
        ...
        gb = torch.exp(-0.5 * (x_theta**2 + gamma**2 * y_theta**2) / sigma**2) * torch.cos(2 * np.pi * x_theta / lambda_ + psi)
        return gb[0]
...
class GaussianFilter:
    def __init__(self, kernel_size=3, sigma=1.0):
        """
        Initialize the GaussianFilter object with a specified kernel size and sigma.
    ...
    def create_gaussian_kernel(self, kernel_size, sigma):
        """
        Create a Gaussian kernel using the specified size and standard deviation.

        Parameters:
        - kernel_size (int): The size of the kernel (must be odd to have a center).
    ...
    def apply_filter(self, input_image):
        ...
        Parameters:
        - input_image (torch.Tensor): A tensor of shape (H, W) or (1, H, W) or (N, C, H, W)

        Returns:
        - torch.Tensor: The filtered image.
        """
        if len(input_image.shape) == 2:
            input_image = input_image.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions if not present

        # Ensure input is floating point for the convolution
        input_image = input_image.float()

        # Apply the Gaussian kernel
        filtered_image = F.conv2d(input_image, self.kernel, padding=self.kernel_size//2)
...
class ErosionFilter:
    ...
    def apply_filter(self, input_image):
        ...
        Parameters:
        - input_image (torch.Tensor): A binary torch.Tensor of shape (H, W) or (1, H, W).
        ...
        # A pixel in the erosion result is 1 only if the sum of the convolution is equal to the area of the kernel
        kernel_area = self.kernel_size ** 2
        eroded_image = (result == kernel_area).float()  # Convert back to binary by checking for full kernel matches
...
def fill_holes(binary_image, kernel_size=3, iterations=10):
    ...
    Args:
    - binary_image (torch.Tensor): Binary image tensor where holes are represented by 0s.
    - kernel_size (int): The size of the square kernel for dilation.
    - iterations (int): Maximum number of iterations to apply dilation to fill holes.

    Returns:
    - torch.Tensor: Binary image with holes filled.
    """
    # Create a structuring element (kernel)
    struct_elem = torch.ones((1, 1, kernel_size, kernel_size), dtype=torch.float32)

    # Start with the original image
    current_image = binary_image.unsqueeze(0).unsqueeze(0)  # Shape [1, 1, H, W]

    # Iterate to fill holes
    for _ in range(iterations):
        # Apply dilation
        dilated_image = F.conv2d(current_image, struct_elem.cuda(), padding=kernel_size//2, stride=1)
